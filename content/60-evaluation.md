...
For memory measurements, the resident set size (RSS) metrics is used instead of the working set size (WSS) [@kerneldocs].
...
However, this can be neglected in this experiment as the transfer only occurs every 10 seconds and includes a short plain text in comparison to the transferred API objects.
Note that these resource measurements only cover the operator processes but not the Kubernetes API server.
Investigating the resource implications of sharding on the control plane components is out of scope of this thesis.
...
The load test is executed twice: for the singleton operator and for the sharded operator.
...
The comparison shows that the sharder's CPU usage is almost as high as the singleton's usage, 88% on average.
The other two shards' CPU usage is about 48% of the singleton's usage on average.
...
- resources are distributed across instances
- usage of shards is a small portion of the singleton's usage
- adding more shards increases capacity and throughput
- sharding enables horizontal scalability of controllers
- however, sharder uses almost as many resources as the singleton
  - doesn't entirely solve the scalability limitations mentioned in [@sec:limitations]
- sharder runs both the shard and sharder components
- idea: run sharder and shards separately to better distribute
- with more and larger objects, difference will be more significant
